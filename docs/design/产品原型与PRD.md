...
| **Framework：LightGBM/XGBoost vs benchmark** | **LightGBM / XGBoost** 对应训练画布：原型为 **6 管道节点**（data source → WOE fit → WOE Transform → Feature selection → Tune & Train → infer），**无 Start/End 占位节点**；实验级配置在顶栏 **Edit Meta / Execute Config**、**ENV**，与 [Feature WideTable](https://github.com/Cedric-Chan/FeatureStore) 一致。扩展 9 节点叙事见 Pipeline-Steps 文档。**benchmark** 画布为 S3 数据源、model_bm、校准等；元信息仍在顶栏。创建 Experiment 时 **Template** 为可选，下拉为当前用户可见的实验名称（Copy 时预填源实验名）；**Framework** 仍由 Model 等字段约束。 |
