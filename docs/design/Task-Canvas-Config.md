...
| 版本 | 画布节点数 | 状态 | 说明 |
|------|-----------|------|------|
| **当前 (MVP)** | **6 节点** | **实现中** | 线性管道：data source → WOE fit → WOE Transform → Feature selection → Tune & Train → inference |
| 未来 (SOP) | 10 节点（含 Pipeline Meta） | 延期 | 合并 WOE All/Selected、独立 Tune/Train、CheckPoint（择优）、Calibrate |

本文 §一 为 Python Step 完整参考（适用所有版本）；§二「合并节点 / 九节点 SOP」描述未来版本设计，**当前实现以 `Pipeline-Steps-and-Canvas-Nodes.md` 第一部分（6 节点 MVP）为准**。
...
画布采用 **合并节点** 设计（对齐建模实验 SOP）：节点 3 为 **WOE All Feature**（对全部特征 woe_fit → woe_transform → woe_merge，再可选 All Feature Report），节点 4 合并 Step 6+3（Feature Selection + Fine Feature Report），节点 5 为 **WOE Selected Feature**（对选中特征可选 woe_update → woe_transform → woe_merge）。**Model Tune**、**Model Train** 拆分为独立节点，支持多子路径做训练参数组合；**CheckPoint（择优）** 含 **Best Select（Model Summary）**，多子路径训练完成后汇总分支结果、用户择优选定，再由 **Model Inference** 使用选定结果执行 model_predict。**CheckPoint** 为**节点属性 `isCheckPoint`、默认关闭**；当 **`isCheckPoint` 节点成功执行完毕**时 Run 进入 **`CHECKING`**，用户 **Review** 后在列表子表或画布 **Action** 选择 **Continue**（回到 **RUNNING** 继续下游）或 **Kill**。Run 状态含 **QUEUING / WAITING / RUNNING / CHECKING / SUCCESS / FAILED / KILLED**（**WAITING** 与 **QUEUING** 同位语）。**Web 原型不实现**「进行中 Run 禁止再 **Trigger Run**」的串行锁；新建 Run 主路径为配置页 **Action → Trigger Run**。Pipeline ENV **`*_checkpoint_after_node`** 为执行器侧节点后暂停语义，**不同于** Run 级 **CHECKING + `isCheckPoint`**。平台支持 **SavePoint** 与 **CheckPoint**。交互上新建执行实例为配置页 **Action → Trigger Run** → **Trigger Run** 弹窗（**Use Cache**、**Run Notes**）；当前导出实现按 **from start** 校验全 DAG。设计意图含「从选中节点起执行」，见 [产品原型与PRD §4.1](./产品原型与PRD.md)。改配置后执行 = 新 Run（按最新 Experiment 配置）。**不修改** [docs/prototype/MODEL_TRAINING.html](../prototype/MODEL_TRAINING.html)。画布内节点**右侧为固定配置面板**，**Tag 分页**：左 **Config**，右 **Last Run** + **Run ID**。
