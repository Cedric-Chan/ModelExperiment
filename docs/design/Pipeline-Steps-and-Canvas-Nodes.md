...
**Model Experiment 原型画布（独立 WOE Fit 节点）— `woe_update` 与 `woe_encoder_path`**：开启 `woe_update` 后，在逐特征更新行**之前**展示 **`woe_encoder_path`**（Pipeline ENV：`woe_fit_woe_encoder_path`）大文本框。默认值与 ENV **为空**时的回显均为**一条真实可引用的 S3 URI**（原型 mock，完整路径见下块，便于在 Git 网页上复制），不使用 `{run_id}` 等模板占位。语义：**读入**该路径的 encoder `.pkl`，按 `woe_fit_woe_updates_json` 做二次加工后，**覆盖写回**本节点约定的 **`encoder_save_path`**（下游 WOE Transform 仍消费该输出）。新 Run 若只想微调 update，可将 S3 路径改为历史某次执行产出的 encoder 对象路径，轻加工后转存为当前 Run 对应的 `encoder_save_path`。
