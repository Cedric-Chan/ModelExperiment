...
*本实体即 **Model Experiment（模型实验）**：定义完整的真实实验计划。基于 **Experiment Template** 的表单提交后，后端生成等价的 Python RayUtil 脚本即该 Experiment。1 Model Version → N Experiment（当前实现为绑定 Model，以产品为准）。Experiment 无状态，仅承载当前/最新画布配置；须绑定已注册 Model；画布配置入口在 Experiment 层级，新建 Run 在画布内点击「Run」执行调度。**暂不支持 cron 调度**。*
...
| status | enum: WAITING / RUNNING / SUCCESS / FAILED / KILLED | Run 状态 |
...
#### 4.2.1 Experiment（无状态）

**Experiment 无状态**，仅承载每次执行 Run 的配置信息（当前/最新画布配置）。画布配置入口在 Experiment 层级；用户从 Experiment 列表进入画布编辑配置，在画布内点击「Run」即新建 Run 并执行调度，Run 携带当前画布配置快照。列表页提供 Edit 等操作，不涉及 Enable/Disable/状态流转。

**与设计稿的对应**：Figma 设计稿（Model Experiment）中的 **TrainingTask** 对应本系统 **Experiment**，**TaskInstance** 对应 **Run**。设计稿中的 Task 状态（DRAFT/ENABLED/DISABLED）、Version/History、Rollback、列表 Trigger/Enable/Disable 均不在本期产品范围内，以本文档为准。Run 状态统一使用 **WAITING**（与设计稿中的 QUEUING 语义一致，仅命名不同）。
...
|------|----------|------|------------------------|
| **WAITING** | `tag-orange` | 创建后等待计算资源 | Kill → 取消排队 |
| **RUNNING** | `tag-blue` | Run 正在执行画布 | Kill → KILLED |
| **SUCCESS** | `tag-green` | Run 执行成功，产物已归档 S3 | — |
| **FAILED** | `tag-red` | Run 执行失败 | — |
| **KILLED** | `tag-red` | Run 被用户终止 | — |

**Run Action 可见性矩阵**：

| 操作 | WAITING | RUNNING | SUCCESS | FAILED | KILLED |
|------|---------|---------|---------|--------|--------|
| View（配置快照+DAG+节点执行） | Yes | Yes | Yes | Yes | Yes |
| View Metrics / Artifacts | 置灰 | 置灰 | Yes | 置灰 | 置灰 |
| View Log | 置灰 | 置灰 | Yes | Yes | Yes |
| Register Build | 置灰 | 置灰 | Yes | 置灰 | 置灰 |
| Kill | Yes（取消排队） | Yes（→ KILLED） | 置灰 | 置灰 | 置灰 |
...
| **模型实验** | Model Experiment (Experiment / EXP) | 完整的真实实验计划。基于 Experiment Template 的表单提交后，后端自动生成等价的 Python RayUtil 脚本，即为该 Model Experiment。绑定已注册 Model，保留当前/最新画布配置；画布入口在 Experiment 层级；一次执行为 Run（Run id 标识）。暂不支持 cron 调度。 | 1 Model Version → N Experiment（以产品为准）；1 Experiment → N Run；继承 Model 的 name 与 region。 |
...
| **CheckPoint** | CheckPoint | 画布节点（Experiment Component）的布尔属性；默认关闭。节点完成后中间产物已写出，Run 状态仅为 WAITING/RUNNING/SUCCESS/FAILED/KILLED（无 CHECKING）。 | 改配置后执行 = 新 Run，从头执行，无变更部分可走缓存。 |
